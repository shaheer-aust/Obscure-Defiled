...
struct Enemy
{
	int hitSoundCooldown = 0;

	void playHitSound()
	{
		if (hitSoundCooldown > 0) return;
		hitSoundCooldown = 20;
		mciSendString("close enemyhitsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//hit.mp3\" alias enemyhitsfx", NULL, 0, NULL);
		mciSendString("play enemyhitsfx from 0", NULL, 0, NULL);
	}

	void playDeathSound()
	{
		mciSendString("close enemydeathsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//death.mp3\" alias enemydeathsfx", NULL, 0, NULL);
		mciSendString("play enemydeathsfx from 0", NULL, 0, NULL);
	}
	...
	void move_enemy(Hero &hero1)
	{
		...
		if (abs(enemyPosition_X - characterX) <= 51 && abs(enemyPosition_Y - characterY) <= 50 && isActive)
		{
			if (hero1.isAttacking)
			{
				...
				enemy_takeDamage(hero1.attack_damage);
			}
			else
			{
		...
		{
			attack_index++;
			if (attack_index >= (int)enemy_attacking_R_images.size())
				attack_index = 0;
			return;
		}
...
struct Boss
{
	...
	void playHitSound()
	{
		if (hitSoundCooldown > 0) return;
		hitSoundCooldown = 20;
		mciSendString("close bosshitsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//hit.mp3\" alias bosshitsfx", NULL, 0, NULL);
		mciSendString("play bosshitsfx from 0", NULL, 0, NULL);
	}

	void playDeathSound()
	{
		mciSendString("close bossdeathsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//death.mp3\" alias bossdeathsfx", NULL, 0, NULL);
		mciSendString("play bossdeathsfx from 0", NULL, 0, NULL);
	}
	...
	{
		if (!isActive) return;
		if (boss_attacking_R_images.empty() || boss_attacking_L_images.empty()) return;
	...
	void show_boss_dead()
	{
		if (boss_dead_R_images.empty() || boss_dead_L_images.empty()) return;
	...
	void show_boss_hit()
	{
		...
		if (hit_index >= (int)boss_hit_R_images.size() - 1)
		{
			hit_index = 0;
			bossGettingHit = false;
		}
	...
	void boss_hit_loop()
	{
		...
		{
			hit_index++;
			if (hit_index >= (int)boss_hit_R_images.size())
			{
				hit_index = 0;
				bossGettingHit = false;
			}
	...
	void move_boss(Hero &hero1)
	{
		...
		if (abs(bossPosition_X - characterX) < 81 && abs(bossPosition_Y - characterY) < 50)
		{
			if (hero1.isAttacking)
			{
				bosstakeDamage(hero1.attack_damage * 0.6);
				hero1.gettingHit = false;
			}
			else
			{
	...
	void update_dead()
	{
		...
		{
			dead_timer++;
			int threshold = (level == 1) ? 16 : 4;
			if (dead_timer > threshold)
			{
				dead_timer = 0;
				dead_index++;
				if (dead_index >= (int)boss_dead_R_images.size())
					dead_index = (int)boss_dead_R_images.size() - 1;
			}
		}
	}
	...
	void update_attack()
	{
		...
		{
			attack_index++;
			if (attack_index >= (int)boss_attacking_R_images.size() - 1)
			{
				isAttacking = false;
				attack_index = 0;
			}
...
struct AlphaBoss
{
	...
	void playHitSound()
	{
		if (hitSoundCooldown > 0) return;
		hitSoundCooldown = 20;
		mciSendString("close alphahitsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//hit.mp3\" alias alphahitsfx", NULL, 0, NULL);
		mciSendString("play alphahitsfx from 0", NULL, 0, NULL);
	}

	void playDeathSound()
	{
		mciSendString("close alphadeathsfx", NULL, 0, NULL);
		mciSendString("open \"resources//sounds//death.mp3\" alias alphadeathsfx", NULL, 0, NULL);
		mciSendString("play alphadeathsfx from 0", NULL, 0, NULL);
	}
	...
	void show_alpha_moving()
	{
		if (alphaHealth <= 0)
		{
			if (dying_R_images.empty() || dying_L_images.empty()) return;
		...
		if (isAttacking)
		{
			...
			if (attackR.empty() || attackL.empty()) return;
		...
		if (walking_R_images.empty() || walking_L_images.empty()) return;
	...
	void move_alpha(Hero &hero1)
	{
		...
		if (alphaHealth <= 0 || !isActive) return;
