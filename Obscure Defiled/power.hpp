...
// ============================================================
//  power.hpp  -  Power-Up System for Obscure Defiled (Level 1)
//
//  Flow:
//  1. A PowerUpIcon is placed on the ground at x=600 in Level 1.
//  2. Every 50-ms tick, PowerUpSystem::update() checks AABB
//     collision between the hero and the icon.
//  3. On collision -> icon vanishes, hero image vectors swap to
//     "Power Up / With Knife" sprites.
//  4. PowerUpSystem::checkRevert() is called each tick.
//     When all Level-1 enemies have 0 health AND hero is powered
//     up, the Normal sprite vectors are restored automatically.
//
//  Integration (GameScreen.hpp & iMain.cpp):
//    GameScreen  -> PowerUpSystem powerUp;
//                   powerUp.init() in initgame_screen(1)
//                   powerUp.draw() in drawgame_screen()
//    iMain.cpp   -> powerUp.shiftIcon() in scroll blocks
//                   powerUp.update() + checkRevert() in 50ms tick
// ============================================================
...
// ─────────────────────────────────────────────────────────────
//  PowerUpIcon  -  drawable / collidable pickup on the ground
// ─────────────────────────────────────────────────────────────
struct PowerUpIcon
{
    double x        = 600;
    ...
    void init(double posX, double posY, double w = 64, double h = 64)
    {
        ...
        isVisible = true;
        textureId = iLoadImage(
            "resources//power_up_icon//damage_up//Power_up_icon.png");
    }
...
struct PowerUpSystem
{
    ...
    void checkRevert(const Enemy& e1, const Enemy& e2,
                     const Boss& boss, Hero& hero)
    {
        if (!isPoweredUp) return;
        if (e1.enemyHealth <= 0 && e2.enemyHealth <= 0 && boss.bossHealth <= 0)
            revert(hero);
    }
